...
export const CreateAssignmentPage: React.FC = () => {
  ...
  const { createAssignment, isCreating } = useAssignmentActions(classroomId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!classroomId || !title.trim() || !dueDate) return;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center gap-3 border-b border-gray-150 dark:border-zinc-800 pb-4">
        ...
        <div>
          <h1 className="text-xl font-black text-gray-900 dark:text-white">Create Classroom Assignment</h1>
          <p className="text-xs text-gray-550 dark:text-zinc-400">Post a new task for your students</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl p-6 space-y-6">
        <div className="space-y-4">
          ...
          <div className="flex items-center pt-2">
            <label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-gray-650 dark:text-zinc-350">
              <input
                type="checkbox"
                checked={allowLate}
        ...
        <div className="flex justify-end gap-3 border-t border-gray-150 dark:border-zinc-800 pt-4">
          ...
          >
            <Check size={16} />
            {isCreating ? 'Creating...' : 'Post Assignment'}
          </button>
        </div>
      </form>
    </div>
